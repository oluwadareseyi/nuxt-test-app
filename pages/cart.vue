<template>
  <main class="container cart">
    <h2>Cart</h2>
    <section class="cartsvg" v-if="!cart.length">
      <svg
        height="100px"
        width="100px"
        fill="#000000"
      ...
      >
        <title id="logo">Empty cart state logo</title>
        <g>
          <g i:extraneous="self">
            <g>
              <path
    ...
    <table v-else>
      ...
      <tbody>
        <tr v-for="item in cart" :key="item.id">
          ...
          <td>
            <span v-for="addon in item.addOn" :key="addon" class="comma">{{
              addon
            }}</span>
          </td>
          <td>{{ item.quantity }}</td>
          <td>{{ item.totalCart }}</td>
        </tr>
        <tr>
          <td colspan="3"></td>
